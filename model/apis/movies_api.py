...
def recommendations(movie_name):
    ...
    for j in range(3):
        movies_data = pd.read_csv('D:\\SenpaiSays\\model\\csvs\\movies.csv')
        ...
        for movie in sorted_similar_movies:
            index = movie[0]
            movie_title = movies_data[movies_data.index == index]['title'].values
...
def suggest():
    ...
    directory_path = "D:\\SenpaiSays\\model\\data"
...
def movie_data1():
    with open('D:\\SenpaiSays\\model\\data\\temp.json', 'r') as f:
        data = json.load(f)
    return jsonify(data)
